...
os.environ["PEECHA_DB_NAME"] = "peecha_test_r189_1"
...
roles_service.set_user_role(user.user_id, manager_role.role_id, company_id, True)

today = datetime.date.today()


def visit_order_and_collect(customer_id, sales_amount, collection_amount):
    visit_id = field_sales_service.start_visit(company_id, customer_id, visitor1.user_id)
    ...
    documents_service.add_line(
        ...
        quantity_base=decimal.Decimal(1), unit_price=decimal.Decimal(sales_amount),
    )
...
visit_order_and_collect(customer_r1, 300000, 100000)
visit_order_and_collect(customer_r2, 500000, 200000)
...
resp = client.get("/manager/dashboard/routes", headers=auth(admin_token))
check(resp.status_code == 200, f"GET /manager/dashboard/routes موفق بود (status={resp.status_code}, body={resp.text})")
route_codes = {r["code"] for r in resp.json()}
check(route_codes == {"R1", "R2"}, f"هر دو مسیر در فهرست هستند (got {route_codes})")
