...
class PayrollRunScreen(FieldHelpMixin, QWidget):
    ...
    def _build_periods_panel(self) -> QWidget:
        ...
        create_period_button = QPushButton("+ دورهٔ تازه")
        create_period_button.setObjectName("primaryButton")
    ...
    def _build_runs_panel(self) -> QWidget:
        ...
        run_button = QPushButton("اجرایِ محاسبه برایِ این دوره")
        run_button.setObjectName("primaryButton")
        run_button.clicked.connect(self._create_and_run)
        layout.addWidget(run_button)

        recalc_button = QPushButton("بازمحاسبهٔ اجرایِ انتخاب‌شده")
        recalc_button.setObjectName("flatButton")
        recalc_button.clicked.connect(self._recalculate_selected)
        layout.addWidget(recalc_button)

        approve_button = QPushButton("تاییدِ نهاییِ اجرا")
        approve_button.clicked.connect(self._approve_selected)
        layout.addWidget(approve_button)

        finalize_button = QPushButton("قطعی‌سازیِ فیش‌ها (فصلِ ۱۴)")
        finalize_button.clicked.connect(self._finalize_selected)
        layout.addWidget(finalize_button)

        post_journal_button = QPushButton("صدورِ سندِ حسابداری (فصلِ ۱۶)")
        post_journal_button.setObjectName("primaryButton")
    ...
    def _build_results_panel(self) -> QWidget:
        ...
        print_button = QPushButton("چاپِ فیشِ انتخاب‌شده")
        ...
        bank_batch_button = QPushButton("تولیدِ فایلِ بانکی (فصلِ ۱۵)")
