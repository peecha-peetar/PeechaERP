...
def _register_font() -> str:
    ...
    اولویت: IRANSans (طبقِ استایلِ مرجعِ کاربر، اگر بعداً اضافه شود) ->
    Vazirmatn -> Tahoma. هر دویِ IRANSans/Vazirmatn قبل از پذیرفته‌شدن با
    نامِ خانواده‌شان اعتبارسنجی می‌شوند (تا فایلِ placeholder/جعلی، مثلِ
    اتفاقی که یک‌بار برایِ Vazirmatn افتاده بود، پذیرفته نشود). طبقِ
    درخواستِ صریحِ کاربر برایِ فونتِ استاندارد و خواناتر: چون «Tahoma»یِ
    واقعی یک فونتِ اختصاصیِ مایکروسافت است و نه رویِ این سرور نصب است و
    نه مجازِ به bundle‌شدن، عملاً فقط یک رشته‌یِ نامِ فونت است که روی
    لینوکس به فونتِ جایگزینِ سیستم (مثلاً DejaVu Sans) برمی‌گردد -- به‌جایش
    فونتِ Vazirmatn (متن‌باز، مجوزِ SIL OFL، طراحی‌شده برایِ فارسی) که از
    نسخه‌یِ ۱۰۹ در assets/fonts/ به‌صورتِ واقعی commit شده، به‌عنوانِ
    استانداردِ فونتِ برنامه استفاده می‌شود."""
