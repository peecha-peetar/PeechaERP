...
# ui/screens/treasury_voucher.py.
#
# طبقِ رفعِ باگِ واقعیِ گزارش‌شده («روش‌هایِ چک و چکِ خرجی که قبلاً وجود
# داشت الان حذف شده»): چک/چکِ‌خرجی این‌جا هم اضافه شدند -- در این مرحله
# فقط روش+مبلغ تعیین می‌شود (نه جزئیاتِ بانک/شماره‌یِ چک)، که دقیقاً با
# همان جزئیات بعداً در خودِ فرمِ دریافت/پرداخت (که با prefill_for_invoice
# باز می‌شود) تکمیل می‌شود -- پس نیازی به دادهٔ اضافه‌ای این‌جا نیست.
# تهاتر/اقساط هم‌چنان مستثنا می‌مانند چون به یک رکوردِ مرتبط (مثلاً
# ردیفِ قسطِ مشخص) وابسته‌اند که این دیالوگ اصلاً برایش طراحی نشده.
...
_SETTLEMENT_PLAN_RECEIPT_METHODS = ("CASH", "BANK", "CHECK", "GOODS_COUPON", "VOUCHER", "DISCOUNT")
_SETTLEMENT_PLAN_PAYMENT_METHODS = ("CASH", "BANK", "CHECK", "CHECK_DISBURSEMENT", "DISCOUNT")
